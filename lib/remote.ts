...
export async function documentText(r:Response,max=4000000){const reader=r.body?.getReader();if(!reader)return '';const chunks:Uint8Array[]=[];let size=0;while(true){const {done,value}=await reader.read();if(done)break;size+=value.length;if(size>max){await reader.cancel();throw new Error('响应内容过大');}chunks.push(value);}const bytes=new Uint8Array(size);let offset=0;for(const c of chunks){bytes.set(c,offset);offset+=c.length;}const header=r.headers.get('content-type')||'',head=new TextDecoder('latin1').decode(bytes.slice(0,2048));const charset=header.match(/charset\s*=\s*["']?([\w-]+)/i)?.[1]||head.match(/(?:charset|encoding)\s*=\s*["']?([\w-]+)/i)?.[1]||(bytes[0]===255&&bytes[1]===254?'utf-16le':bytes[0]===254&&bytes[1]===255?'utf-16be':'utf-8');try{return new TextDecoder(charset).decode(bytes);}catch{return new TextDecoder().decode(bytes);}}
