...
import {INSTANCE_SNAPSHOT,importInstances,mergeInstances,type PublicInstance} from '@/lib/rsshub-instances';
import {isNative} from '@/lib/api-client';
import {DeviceInstance} from '@/components/device-instance';
...
export function RSSHubInstances({route,value,onChange}:{route:string;value:string;onChange:(v:string)=>void}){
 ...
 const [custom,setCustom]=useState<PublicInstance[]>([]),[input,setInput]=useState(''),[query,setQuery]=useState('');
 useEffect(()=>{try{const list=JSON.parse(localStorage.getItem('yueliu-own-instances')||'[]');setCustom(list);setRows(mergeInstances(INSTANCE_SNAPSHOT,list).map(r=>({...r,status:'pending'})));}catch{}},[]);
 function saveCustom(next:PublicInstance[]){setCustom(next);localStorage.setItem('yueliu-own-instances',JSON.stringify(next));setRows(prev=>mergeInstances(prev,next).map(r=>({...r,status:prev.find(p=>p.url===r.url)?.status||'pending'})));}
 const controller=useRef<AbortController|null>(null);
 useEffect(()=>()=>controller.current?.abort(),[]);
 const shown=rows.filter(r=>(r.url+' '+r.location+' '+r.maintainer).toLowerCase().includes(query.toLowerCase()));
 ...
 async function refresh(){
  ...
  try{
   const response=await apiFetch('/api/rsshub/instances',{cache:'no-store',signal:ctl.signal});if(!response.ok)throw new Error('实例名单获取失败');let catalog:Catalog=await response.json();
   ...
   const list=mergeInstances(catalog.instances,custom);setRows(list.map(r=>({...r,status:'pending'})));let next=0;
 ...
 return <section className="instance-panel"><DeviceInstance onUse={url=>{saveCustom(mergeInstances(custom,[{url,location:'本机',maintainer:'我的设备'}]));onChange(url);}}/><details className="own-instance-import"><summary>我的实例 · 批量导入 / 导出（{custom.length}）</summary><label className="field">每行一个实例地址<textarea rows={4} value={input} onChange={e=>setInput(e.target.value)} placeholder="https://your-rsshub.example.com"/></label><button className="subtle" disabled={!input.trim()||running} onClick={()=>{try{const imported=importInstances(input,isNative());saveCustom(mergeInstances(custom,imported));setInput('');toast.success('已导入 '+imported.length+' 个实例，请检测后使用');}catch(e){toast.error((e as Error).message);}}}>导入列表</button><button className="subtle" onClick={()=>{setInput(custom.map(r=>r.url).join('\n'));}}>导出到输入框</button>{custom.map(r=><div className="own-instance-row" key={r.url}><code>{r.url}</code><button disabled={running} onClick={()=>{saveCustom(custom.filter(x=>x.url!==r.url));setRows(rows=>rows.filter(x=>x.url!==r.url));}}>移除</button></div>)}</details><label className="field">搜索节点<input value={query} onChange={e=>setQuery(e.target.value)} placeholder="域名、地区、维护者"/></label><div className="instance-heading"><h3><Server size={17}/>公共实例</h3><button className="instance-refresh" type="button" disabled={running||!route.trim()} onClick={refresh}>{running?<Loader2 size={15} className="spin"/>:<RefreshCw size={15}/>} {running?'正在检测…':'刷新并检测可用实例'}</button></div><p className="instance-caption">从文档名单发现实例，并实际请求当前路由。汇总官网、官方源码、社区公开名单和你的实例。每个节点最多等待 12 秒。</p><div className="instance-route">测试路由：<code>{route||'请先填写公众号微信号'}</code></div>
