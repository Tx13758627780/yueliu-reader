...
for(const file of ['MainActivity.java','YueliuFilesPlugin.java','BackExitGate.java','UpdatePolicy.java','YueliuUpdaterPlugin.java','YueliuUpdateFileProvider.java'])copyFileSync('native/android-src/'+file,java+'/'+file);
...
let updatedManifest=readFileSync(manifest,'utf8');
if(!updatedManifest.includes('android.permission.REQUEST_INSTALL_PACKAGES'))updatedManifest=updatedManifest.replace('</manifest>','    <uses-permission android:name="android.permission.REQUEST_INSTALL_PACKAGES" />\n</manifest>');
if(!updatedManifest.includes('${applicationId}.updater'))updatedManifest=updatedManifest.replace('</application>','<provider android:name="app.yueliu.reader.YueliuUpdateFileProvider" android:authorities="${applicationId}.updater" android:exported="false" android:grantUriPermissions="true"><meta-data android:name="android.support.FILE_PROVIDER_PATHS" android:resource="@xml/updater_paths" /></provider></application>');
writeFileSync(manifest,updatedManifest);
writeFileSync('native/android/app/src/main/res/xml/updater_paths.xml','<?xml version="1.0" encoding="utf-8"?><paths><external-files-path name="updates" path="Download/updates/" /></paths>');
