import {biliContentId,fetchBiliContent} from '@/lib/bili-content';
...
export async function GET(req:Request){
 try{
  ...
  if(biliContentId(u.href))return Response.json(await fetchBiliContent(u.href,AbortSignal.any([req.signal,AbortSignal.timeout(25000)])),{headers:{'Cache-Control':'no-store'}});
  ...
  return Response.json({html:await documentText(r),url:r.url||u.href},{headers:{'Cache-Control':'no-store'}});
